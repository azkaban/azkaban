...
//    setTestOnBorrow(true);
    ...
    // This set's the auto retry if a connection fails to initialize
  }

  /**
   * This method overrides {@link HikariDataSource#getConnection()}, in order to have retry logics.
...
//    }
    try {
      /**
       * when DB connection could not be fetched (e.g., network issue), or connection can not be validated,
       * {@link HikariDataSource} throws a SQL Exception. {@link HikariDataSource#dataSource} will be reset to
       * null.
       * createDataSource() will create a new dataSource.
       * Every Attempt generates a thread-hanging-time, about 75 seconds, which is hard coded, and can not be changed.
       */
      ...
      logger.error("Failed to find write-enabled DB connection with Hikari connection pool");
    }
    return connection;
  }
