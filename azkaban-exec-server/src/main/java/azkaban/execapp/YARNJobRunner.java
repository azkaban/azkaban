...
public class YARNJobRunner extends EventHandler implements Runnable {

  public static final String AZKABAN_WEBSERVER_URL = "azkaban.webserver.url";

  private static final Logger serverLogger = Logger.getLogger(YARNJobRunner.class);
  ...
  public YARNJobRunner(final ExecutableNode node, final File workingDir, final ExecutorLoader loader,
  ...
  public static String createLogFileName(final ExecutableNode node) {
    return YARNJobRunner.createLogFileName(node, node.getAttempt());
  }
  ...
  public static String createMetaDataFileName(final ExecutableNode node) {
    return YARNJobRunner.createMetaDataFileName(node, node.getAttempt());
  }

  public static String createAttachmentFileName(final ExecutableNode node) {

    return YARNJobRunner.createAttachmentFileName(node, node.getAttempt());
  }
  ...
  /**
   * Returns a list of jobs that this YARNJobRunner will wait upon to finish before starting. It is only
  ...
  private FileAppender createFileAppender() throws IOException {
    ...
    final FileAppender fileAppender =
        new FileAppender(this.loggerLayout, absolutePath, true);
    ...
    try {
      final File[] files = this.logFile.getParentFile().listFiles(new FilenameFilter() {
        ...
        public boolean accept(final File dir, final String name) {
          return name.startsWith(YARNJobRunner.this.logFile.getName());
        }
      });
  ...
  private void doRun() {
    Thread.currentThread().setName(
        "YARNJobRunner-" + this.jobId + "-" + this.executionId);
  ...
  private void logError(final String message) {
    this.logger.error("Deepak : Using Flow Logger : " + message);
  }

  private void logError(final String message, final Throwable t) {
    this.logger.error("Deepak : Using Flow Logger : " + message, t);
  }

  private void logInfo(final String message) {
    this.logger.info("Deepak : Using Flow Logger : " + message);
  }
