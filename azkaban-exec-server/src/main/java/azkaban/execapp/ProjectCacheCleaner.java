...
class ProjectCacheCleaner {
  ...
  ProjectCacheCleaner(final File projectDir, final long projectDirMaxSizeInMb) {
    Preconditions.checkNotNull(projectDir);
    Preconditions.checkArgument(projectDir.exists());
    Preconditions.checkArgument(projectDirMaxSizeInMb > 0);
  ...
  private List<Path> loadAllProjectDirs() {
    ...
    for (final File project : this.projectDir.listFiles(new FilenameFilter() {
  ...
  private long getProjectDirsTotalSizeInBytes(final List<ProjectDirectoryMetadata> allProjects) {
    ...
    for (final ProjectDirectoryMetadata proj : allProjects) {
      totalSizeInBytes += proj.getDirSizeInBytes();
    }
  ...
  void deleteProjectDirsIfNecessary(final long spaceToDeleteInBytes) {
    ...
    if (currentSpaceInBytes + spaceToDeleteInBytes
      ...
      log.info(
          ...
          (currentSpaceInBytes + spaceToDeleteInBytes) / (1024 * 1024),
          this.projectDirMaxSizeInMb);
