		...
		.extend({
			...
			initFromSched : function(scheduleId) {
				...
				var successHandler = function(data) {
					...
					// data bag.
					if (data.NyxTriggerChecker_1 != undefined
							&& data.NyxTriggerChecker_1 != null
						...
						var triggerName = trigger.name ? trigger.name
								: "unknown";
						var triggerOwner = trigger.owner ? trigger.owner
								: "unknown";
						var triggerCreatedTM = trigger.createdTs ? (new Date(
								trigger.createdTs)).toString() : "unknown";
						var triggerUpdatedTM = trigger.updatedTs ? (new Date(
								trigger.updatedTs)).toString() : "unknown";

						if (trigger.ruleSet != undefined
								&& trigger.ruleSet != null) {
							// retrieve the rule relation from the expression
							// field.
							ruleRel = trigger.ruleSet.expression ? trigger.ruleSet.expression
									: ruleRel;

							// fill in the list for the rules.
							if (trigger.ruleSet.ruleList != undefined
									&& trigger.ruleSet.ruleList != null) {
								...
								for ( var idx in ruleList) {
									...
									row.insertCell().innerHTML = ruleList[idx].ready ? "<span class='label label-success'>Ready</span>"
											: "<span class='label label-danger'>NotReady</span>";
									row.insertCell().innerText = ruleList[idx].updatedTs ? (new Date(
											ruleList[idx].updatedTs))
											.toString()
											: "unknown";
								}
							}
				...
				$.get(this.scheduleURL, fetchTriggerData, successHandler,
						"json");
			},
