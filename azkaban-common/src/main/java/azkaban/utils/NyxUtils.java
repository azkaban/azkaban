...
public class NyxUtils {
	...
	public static long registerNyxTrigger(String specificationJson)
		...
		try {
			...
			headers.add(new BasicNameValuePair("Content-Type",
					"application/json"));
			String rawResponse = client.httpPost(uri, headers,
					specificationJson);
			Map<String, Object> parsedResponse = (Map<String, Object>) JSON
					.parse(rawResponse);
	...
	public static void unregisterNyxTrigger(Long triggerId)
			throws TriggerManagerException {
		if (triggerId == -1) {
			logger.info("skipping unregistering nyx trigger as the trigger is not yet registered with service.");
			return;
		}
	...
	public static boolean isNyxTriggerReady(Long triggerId)
		...
		try {
			...
			if (parsedResponse.containsKey("error")) {
				...
				throw new Exception("Status missing from Nyx response :"
						+ response);
			}
		} catch (Exception ex) {
			logger.error("Failed to get Nyx service response for triggerId : "
