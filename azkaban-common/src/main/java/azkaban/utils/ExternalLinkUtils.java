...
public class ExternalLinkUtils {
   ...
   */
  public static List<ExternalAnalyzer> getExternalAnalyzers( final Props azkProps, final HttpServletRequest req) {
    ...
    return externalTopics.stream().map(topic -> {
      ...
      if (!externalLinkUrl.isEmpty()) {
        ...
        return new ExternalAnalyzer(topic, execExternalLinkLabel, externalLinkUrl, validExternalLink(externalLinkUrl));
      }
      return null;
   ...
   */
  private static boolean validExternalLink(final String externalLinkUrl) {
    ...
    try {
      ...
      logger.info("validExternalLink url " + externalLinkUrl + " is reachable.");
      client.close();
      return true;
    } catch (IOException e) {
