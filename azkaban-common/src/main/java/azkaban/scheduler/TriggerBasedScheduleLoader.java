...
public class TriggerBasedScheduleLoader implements ScheduleLoader {
	...
	private Trigger scheduleToTrigger(Schedule s) {
		...
		Trigger t = new Trigger(s.getScheduleId(), s.getLastModifyTime(),
	...
	private List<TriggerAction> createActions(Schedule s) {
		...
		ExecuteFlowAction executeAct = new ExecuteFlowAction(
				"executeFlowAction", s.getProjectId(), s.getProjectName(),
				s.getFlowName(), s.getSubmitUser(), s.getExecutionOptions(),
				s.getSlaOptions());
	...
	private Condition createTriggerCondition(Schedule s) {
		...
		ConditionChecker checker = new BasicTimeChecker("BasicTimeChecker_1",
				s.getFirstSchedTime(), s.getTimezone(), s.isRecurring(),
				s.skipPastOccurrences(), s.getPeriod());
		checkers.put(checker.getId(), checker);
		String expr = checker.getId() + ".eval()";

		Map<String, String> flowParams = s.getExecutionOptions()
				.getFlowParameters();
		if (flowParams != null
	...
	private Condition createExpireCondition(Schedule s) {
		Map<String, ConditionChecker> checkers = new HashMap<String, ConditionChecker>();
		ConditionChecker checker = new BasicTimeChecker("BasicTimeChecker_2",
				s.getFirstSchedTime(), s.getTimezone(), s.isRecurring(),
				s.skipPastOccurrences(), s.getPeriod());
		checkers.put(checker.getId(), checker);
		String expr = checker.getId() + ".eval()";
		...
		if (flowParams != null
				&& flowParams.containsKey(ExecutionOptions.TRIGGER_SPEC)) {
			ConditionChecker nyxChecker = new NyxTriggerChecker(
					flowParams.get(ExecutionOptions.TRIGGER_SPEC), expCheckerId);
			checkers.put(nyxChecker.getId(), nyxChecker);
			expr = expr + " && " + nyxChecker.getId() + ".eval() ";
		}
	...
	public void insertSchedule(Schedule s) throws ScheduleManagerException {
		...
		try {
			...
			throw new ScheduleManagerException(
					"Failed to insert new schedule!", e);
		}
	}
	...
	private Schedule triggerToSchedule(Trigger t)
			throws ScheduleManagerException {
		...
		if (ck != null && act != null) {
			Schedule s = new Schedule(t.getTriggerId(), act.getProjectId(),
					act.getProjectName(), act.getFlowName(), t.getStatus()
							.toString(), ck.getFirstCheckTime(),
					ck.getTimeZone(), ck.getPeriod(), t.getLastModifyTime(),
					ck.getNextCheckTime(), t.getSubmitTime(),
					t.getSubmitUser(), act.getExecutionOptions(),
					act.getSlaOptions());
			return s;
		} else {
	...
	public synchronized List<Schedule> loadUpdatedSchedules()
		...
		try {
			triggers = triggerManager.getTriggerUpdates(triggerSource,
					lastUpdateTime);
		} catch (TriggerManagerException e) {
			e.printStackTrace();
