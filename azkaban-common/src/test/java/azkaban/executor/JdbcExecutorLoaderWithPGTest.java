...
@Ignore
public class JdbcExecutorLoaderWithPGTest {
  ...
  public void testFetchQueuedFlows() throws ExecutorManagerException,
    ...
    Pair<ExecutionReference, ExecutableFlow>[] after = loader.fetchQueuedFlows().toArray(new Pair[0]);
    //Arrays.sort(after);
    Pair<ExecutionReference, ExecutableFlow>[] before = queuedFlows.toArray(new Pair[0]);
    // only running and completed flows
    Assert.assertEquals(after.length,
      before.length);
  }
  ...
  public void testFetchAllExecutors() throws Exception {
    ...
    Executor[] before = executors.toArray(new Executor[0]);
    Executor[] after = fetchedExecutors.toArray(new Executor[0]);
  ...
  public void testActiveReference() throws Exception {
    ...
    ExecutorLoader loader = createLoader();
    executor = loader.addExecutor(executor.getHost(), executor.getPort());
