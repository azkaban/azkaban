...
@Ignore
public class JdbcProjectLoaderWithPGTest {
  ...
  public static void setupDB() {
    DataSource dataSource =
        DataSourceUtils.getPostgresqlDataSource(host, port, database, user,
  ...
  private static void clearDB() {
    ...
    public void testPermissionRetrivalByFetchProjectByName()
        ...
        if(project.getGroupPermission("group") != null){
        	project.removeGroupPermission("group");
        }
    ...
    public void testDuplicateRetrivalByFetchProjectByName()
        ...
        //Project fetchedProject = loader.fetchProjectByName(project.getName());
        List<Project> projects = loader.fetchAllActiveProjects();
        boolean exists = false;
        for(Project activeProject: projects){
        	if(activeProject.getName().equals(newProject.getName())
        			&& activeProject.getId() == newProject.getId()){
        		exists = true;
        	}
        }
  ...
  public void testAddRemovePermissions() throws ProjectManagerException {
    ...
    Permission permOverride = new Permission(0x6);
    if(project.getGroupPermission("group1") != null){
    	project.removeGroupPermission("group1");
    }
    loader.updatePermission(project, user.getUserId(), permOverride, false);
    Assert.assertEquals(permOverride,
  ...
  public void testProjectFilesUpload() throws ProjectManagerException {
    ...
    File testDir = new File("C:\\Users\\Administrator\\git\\azkaban\\azkaban-common\\src\\test\\resources\\project\\testjob\\testjob.zip");
