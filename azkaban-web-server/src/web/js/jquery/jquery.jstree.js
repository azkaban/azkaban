    ...
    _fn: {
      set_lang: function (i) {
        ...
        st = $.vakata.css.get_css(
            selector + "." + this.data.languages.current_language, false,
...
(function ($) {
  ...
  $.vakata.dnd = {
    ...
    drag: function (e) {
      ...
      if (!$.vakata.dnd.is_drag) {
        if (Math.abs(e.pageX - $.vakata.dnd.init_x) > 5 || Math.abs(
                e.pageY - $.vakata.dnd.init_y) > 5) {
    ...
    _fn: {
      ...
      start_drag: function (obj, e) {
        ...
        $.vakata.dnd.drag_start(e, {jstree: true, obj: o},
            "<ins class='jstree-icon'></ins>" + (o.length > 1
                ? "Multiple selection" : this.get_text(o)));
        if (this.data.themes) {
          m.attr("class", "jstree-" + this.data.themes.theme);
          $.vakata.dnd.helper.attr("class",
              "jstree-dnd-helper jstree-" + this.data.themes.theme);
          ...
          a.each(function () {
            ...
            if (tmp1.children("ins").get(0).className.replace(/jstree[^ ]*|$/ig,
              ...
              result += ' icon="' + tmp1.children("ins").get(
                      0).className.replace(/jstree[^ ]*|$/ig, '').replace(
                      /^\s+$/ig, "") + '"';
            }
    ...
    _fn: {
      search: function (str, skip_async) {
        ...
        this.data.search.result = this.get_container().find(
            "a" + (this.data.languages ? "." + this.get_lang() : "" ) + ":"
