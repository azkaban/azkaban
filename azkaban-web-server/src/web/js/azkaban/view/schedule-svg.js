...
$(function () {
  ...
  var monthConst = ["January", "February", "March", "April", "May", "June",
    "July", "August", "September", "October", "November", "December"];
  var dayOfWeekConst = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday",
  ...
  $("#svgDivCustom").svg({
    onLoad: function (svg) {
      ...
      //Outer g
      var gMain = svg.group({
        transform: "translate(" + border + ".5," + border + ".5)",
        stroke: "#999",
        strokeWidth: 1
      });
      ...
      if (window.location.hash) {
        try {
          ...
          var newDate = new Date(parseInt(dateParts[0]),
              parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));
          if (!isNaN(newDate)) {
            svgDate = newDate;
          }
        }
      ...
      $(".nav-prev-week").click(function (event) {
        ...
        window.location.hash = "#" + svgDate.getFullYear() + "-"
            + (svgDate.getMonth() + 1) + "-" + svgDate.getDate();
        loadSvg(svgDate);
      ...
      function loadSvg(firstDay) {
        ...
        var monthIndicatorText = monthConst[firstDay.getMonth()] + " "
            + firstDay.getFullYear().toString();
        //Measure a good width for the text to display well
        timeWidth = Math.max(minTimeWidth, measureText(svg, monthIndicatorText,
            {fontSize: "20", style: "text-anchor: end;"}));

        var dayWidth = Math.floor(
            (totalWidth - 3 * border - timeWidth) / numDays);
        ...
        svg.text(gMain, timeWidth, header - 8, monthIndicatorText, {
          fontSize: "20",
        ...
        for (var i = 0; i < 24; i++) {
          ...
          svg.text(gLeft, timeWidth, i * lineHeight + 15, hourText, {
            fontSize: "14",
            style: "text-anchor: end;",
            fill: "#333",
            stroke: "none"
        ...
        firstDay = new Date(firstDay.getFullYear(), firstDay.getMonth(),
            firstDay.getDate()).valueOf();
        var isThisWeek = -1;
        //Draw background
        for (var deltaDay = 0; deltaDay < numDays; deltaDay++) {
          //Day group
          var gDay = svg.group(gMain, {
            transform: "translate(" + (border + timeWidth + deltaDay * dayWidth)
            + "," + header + ")"
          });
          ...
          var isToday = date.getFullYear() == today.getFullYear()
              && date.getMonth() == today.getMonth() && date.getDate()
              == today.getDate();
          if (isToday) {
            isThisWeek = deltaDay;
          }
          svg.rect(gDay, 0, -header, dayWidth, 24 * lineHeight + header,
              {fill: "none", stroke: "#F60"});
          //Draw day title
          svg.text(gDay, 6, -8, day + " " + dayOfWeekConst[date.getDay()], {
            fontSize: "20",
            fill: isToday ? "#06C" : "#F60",
            stroke: "none"
          });
        ...
        var gDayView = svg.group(gDayViewOuterGroup, {
          transform: "translate(" + (border + timeWidth) + "," + header + ")"
        });
        if (isThisWeek != -1) {
          var date = new Date(firstDay + dayMillisConst * isThisWeek);
          var day = date.getDate();
          var gDay = svg.group(gMain, {
            transform: "translate(" + (border + timeWidth + isThisWeek
            * dayWidth) + "," + header + ")"
          });
          svg.rect(gDay, 0, -header, dayWidth, 24 * lineHeight + header,
              {fill: "none", stroke: "#06F"});
          var lineY = Math.floor(
              today.getHours() * lineHeight + today.getMinutes() * lineHeight
              / 60);
          svg.line(gDay, 0, lineY, dayWidth, lineY,
              {fill: "none", stroke: "#06F", strokeWidth: 4});
        }
        ...
        function filterApplies(item) {
          ...
          for (var i = 0; i < filterFlow.length; i++) {
            if (item.projectname == filterFlow[i].projectname && item.flowname
                == filterFlow[i].flowname) {
              return true;
            }
          }
          return false;
        ...
        function renderDays() {
          ...
          svg.remove(gDayView);
          gDayView = svg.group(gDayViewOuterGroup, {
            transform: "translate(" + (border + timeWidth) + "," + header + ")"
          });

          //Add day groups
          for (var deltaDay = 0; deltaDay < numDays; deltaDay++) {
            var gDay = svg.group(gDayView,
                {transform: "translate(" + (deltaDay * dayWidth) + ")"});
            ...
            data.sort(function (a, b) {
              ...
              if (timeDiff == 0) {
                ...
                if (lengthDiff == 0) {
                  ...
                  return (a.flowname < b.flowname ? 1 : a.flowname > b.flowname
                      ? -1 : 0);
                }
                return lengthDiff;
              }
            ...
            for (var i = 0; i < columns.length; i++) {
              //Split into columns
              var gColumn = svg.group(gDay, {
                transform: "translate(" + Math.floor(
                    i * dayWidth / columns.length) + ")", style: "opacity: 0.8"
              });
              for (var j = 0; j < columns[i].length; j++) {
                ...
                var startY = Math.floor(
                    startTime.getHours() * lineHeight + startTime.getMinutes()
                    * lineHeight / 60);
                var endTime = new Date(item.time + item.length);
                var endY = Math.ceil(
                    startY + (item.length * lineHeight) / hourMillisConst);
                var deltaY = Math.ceil(endY - startY);
                if (deltaY < 5) {
                  deltaY = 5;
                }
                //var anchor = svg.a(gColumn);
                var itemUrl = contextURL + "/manager?project="
                    + item.projectname + "&flow=" + item.flowname;
                var gItem = svg.link(gColumn, itemUrl,
                ...
                var rect = svg.rect(gItem, 0, 0,
                    Math.ceil(dayWidth / columns.length), deltaY, 0, 0,
                    {fill: item.item.color, stroke: "#444", strokeWidth: 1});
        ...
        function processItem(item, scheduled) {
          ...
          if (period > 0) {
            ...
            var periods = Math.floor(
                (restrictedStartTime - (firstTime)) / period);
        ...
        function addItem(obj) {
          ...
          var itemStartDate = new Date(itemStartTime.getFullYear(),
              itemStartTime.getMonth(), itemStartTime.getDate());
          var itemEndDate = new Date(itemEndTime.getFullYear(),
              itemEndTime.getMonth(), itemEndTime.getDate());

          //Cross date item, cut it to only today's portion and add another item starting tomorrow morning
          if (itemStartDate.valueOf() != itemEndDate.valueOf()
              && itemEndTime.valueOf() != itemStartDate + dayMillisConst) {
        ...
        function handleContextMenu(event) {
          ...
          var menu = [
            {
              title: "Job \"" + item.flowname + "\" From Project \""
              + item.projectname + "\""
            },
            {
              title: "View Job", callback: function () {
              window.location.href = requestURL;
            }
            },
            {
              title: "View Job in New Window", callback: function () {
              window.open(requestURL);
            }
            },
            {
              title: "Hide Job", callback: function () {
              filterFlow.push(item);
              renderDays();
            }
            },
            {
              title: "Hide All Jobs From the Same Project",
              callback: function () {
        ...
        function handleMouseOver(event) {
          ...
          var tooltip = svg.group({
            transform: "translate(" + (thisOffset.left - offset.left + 2) + ","
            + (thisOffset.top - offset.top - 2) + ")"
          });
          var text = [
          ...
          var textLength = Math.max(measureText(svg, text[0], {fontSize: "13"}),
              measureText(svg, text[1], {fontSize: "13"}));
          var rect = svg.rect(tooltip, 0, -40, textLength + 4, 40,
              {fill: "#FFF", stroke: "none"});
          svg.text(tooltip, 2, -25, text[0],
              {fontSize: "13", fill: "#000", stroke: "none"});
          svg.text(tooltip, 2, -5, text[1],
              {fontSize: "13", fill: "#000", stroke: "none"});
        ...
        function handleMouseOut(event) {
          ...
          $($(this).data("tooltip")).fadeOut(250, function () {
            svg.remove(this);
          });
        }
        ...
        for (var deltaDay = 0; deltaDay < numDays; deltaDay++) {
          $.ajax({
            ...
            data: {
              "ajax": "loadHistory",
              "startTime": firstDay + deltaDay * dayMillisConst,
              "loadAll": 0
            },
            //dataType: "json",
            success: function (data) {
              ...
              for (var i = 0; i < items.length; i++) {
                ...
                items[i].objs = new Array();
                items[i].color = "#7E7";
                if (items[i].status == 60 || items[i].status == 70
                    || items[i].status == 80) {
                  items[i].color = "#E77";
                }
  ...
  function getHourText(hour) {
    return (hour == 0 ? "12 AM" : (hour < 12 ? hour + " AM" : (hour == 12
        ? "12 PM" : (hour - 12) + " PM" )));
  }
  ...
  function formatReadablePeriod(period) {
    ...
    var text = "";
    if (days > 0) {
      text = (days == 1 ? "24 hours" : days.toString() + " days");
    }
    if (hours > 0) {
      text = text + " " + (hours == 1 ? "1 hour" : hours.toString()
              + " hours");
    }
    if (mins > 0) {
      text = text + " " + (mins == 1 ? "1 minute" : mins.toString()
              + " minutes");
    }
    return text;
  }
