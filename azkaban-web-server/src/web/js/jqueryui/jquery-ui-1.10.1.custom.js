      ...
      if (($.ui.ie && (/(static|relative)/).test(this.css("position")))
        ...
        scrollParent = this.parents().filter(function () {
          return (/(relative|absolute|fixed)/).test($.css(this, "position"))
              && (/(auto|scroll)/).test(
                  $.css(this, "overflow") + $.css(this, "overflow-y") + $.css(
                      this, "overflow-x"));
        }).eq(0);
      } else {
        scrollParent = this.parents().filter(function () {
          return (/(auto|scroll)/).test(
              $.css(this, "overflow") + $.css(this, "overflow-y") + $.css(this,
                  "overflow-x"));
        }).eq(0);
      }
    ...
    _generatePosition: function (event) {
      ...
      if (this.originalPosition) { //If we are not dragging yet, we won't check for options
        ...
        if (o.grid) {
          ...
          >= containment[0] || left - this.offset.click.left > containment[2])
              ? left : ((left - this.offset.click.left >= containment[0]) ? left
...
(function ($, undefined) {
  ...
  $.ui.plugin.add("resizable", "containment", {
    ...
    resize: function (event) {
      ...
      woset = Math.abs(
          (that._helper ? that.offset.left - cop.left : (that.offset.left
          - cop.left)) + that.sizeDiff.width);
      hoset = Math.abs(
          (that._helper ? that.offset.top - cop.top : (that.offset.top
...
(function ($, undefined) {
  ...
  $.widget("ui.sortable", $.ui.mouse, {
    ...
    _intersectsWithPointer: function (item) {
      ...
      var isOverElementHeight = (this.options.axis === "x") || isOverAxis(
                  this.positionAbs.top + this.offset.click.top, item.top,
              item.height),
          isOverElementWidth = (this.options.axis === "y") || isOverAxis(
      ...
      if (!o.placeholder || o.placeholder.constructor === String) {
        ...
        o.placeholder = {
          ...
          update: function (container, p) {
            ...
            if (!p.height()) {
              p.height(that.currentItem.innerHeight() - parseInt(
                      that.currentItem.css("paddingTop") || 0, 10) - parseInt(
                      that.currentItem.css("paddingBottom") || 0, 10));
            }
            if (!p.width()) {
              p.width(that.currentItem.innerWidth() - parseInt(
                      that.currentItem.css("paddingLeft") || 0, 10) - parseInt(
                      that.currentItem.css("paddingRight") || 0, 10));
            }
          }
        };
      }
    ...
    _contactContainers: function (event) {
      ...
      if (this.containers.length === 1) {
        ...
        for (j = this.items.length - 1; j >= 0; j--) {
          ...
          nearBottom = false;
          if (Math.abs(cur - base) > Math.abs(
                  cur + this.items[j][sizeProperty] - base)) {
            nearBottom = true;
  ...
  $.extend(Datepicker.prototype, {
    ...
    _checkOffset: function (inst, offset, isFixed) {
      ...
      offset.left -= Math.min(offset.left,
          (offset.left + dpWidth > viewWidth && viewWidth > dpWidth) ? Math.abs(
              offset.left + dpWidth - viewWidth) : 0);
              ...
              case "o":
                output += formatNumber("o",
                    Math.round((new Date(date.getFullYear(), date.getMonth(),
                            ...
                            date.getFullYear(), 0, 0).getTime()) / 86400000),
                    3);
                break;
              case "m":
...
(function ($) {
  ...
  $.widget("ui.spinner", {
    ...
    _increment: function (i) {
      ...
      if (incremental) {
        return $.isFunction(incremental) ? incremental(i) : Math.floor(
...
// based on easing equations from Robert Penner (http://www.robertpenner.com/easing)
    ...
    $.extend(baseEasings, {
      ...
      Elastic: function (p) {
        return p === 0 || p === 1 ? p : -Math.pow(2, 8 * (p - 1)) * Math.sin(
