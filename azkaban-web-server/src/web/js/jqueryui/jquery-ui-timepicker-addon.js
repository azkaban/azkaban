      ...
      if (adjustSliders !== undefined && adjustSliders === true) {
        var hourMax = parseInt(
            ...
            minMax = parseInt(
                ...
                - this._defaults.minuteMin) % this._defaults.stepMinute)), 10),
            secMax = parseInt(
                (this._defaults.secondMax - ((this._defaults.secondMax
                - this._defaults.secondMin) % this._defaults.stepSecond)), 10),
            millisecMax = parseInt(
                (this._defaults.millisecMax - ((this._defaults.millisecMax
                - this._defaults.millisecMin) % this._defaults.stepMillisec)),
                10);
    ...
    _controls: {
      ...
      select: {
        ...
        options: function (tp_inst, obj, unit, opts, val) {
          ...
          return tp_inst.control.create(tp_inst, obj, $t.data('unit'), $t.val(),
              o.min || $t.data('min'), o.max || $t.data('max'),
              o.step || $t.data('step'));
        },
        value: function (tp_inst, obj, unit, val) {
  ...
  $.datepicker.parseTime = function (timeFormat, timeString, options) {
    ...
    var strictParse = function (f, s, o) {
      ...
      var regstr = '^' + f.toString()
              .replace(/([hH]{1,2}|mm?|ss?|[tT]{1,2}|[lz]|'.*?')/g,
                  function (match) {
        ...
        if (!tp_inst._defaults.timeOnly
          ...
          if (tp_inst._defaults.altFormat) {
            altFormattedDateTime = $.datepicker.formatDate(
                    ...
                    (date === null ? new Date() : date), formatCfg)
  ...
  $.datepicker.parseDate = function (format, value, settings) {
    ...
    try {
      ...
      date = this._base_parseDate(format, value.substring(0,
          value.length - (err.length - err.indexOf(':') - 2)), settings);
      $.datepicker.log(
